...
export default async function handler(
  ...
  const searchQuery = filter.query?.replaceAll(`'`, `''`) || "";

  const prisma = new PrismaClient();

  const searchRelevanceSql = `CASE WHEN UPPER(otherTitles) LIKE UPPER('%"${searchQuery}"%') THEN 10 ELSE 0 END
  + CASE WHEN UPPER(otherTitles) LIKE UPPER('%${searchQuery}%') THEN 5 ELSE 0 END
  ...
  // const tagsOR = filter?.tags?.map((tag) => `UPPER(TagCN.name)=UPPER('${tag}') OR UPPER(TagCN.nameEN)=UPPER('${tag}'))`)
  //   .join(" OR ")
  ...
  `${filter.tags && filter.tags.length > 0 ? `\nAND id IN (SELECT id FROM Donghua JOIN DonghuaTagCN JOIN TagCN ON Donghua.id = DonghuaTagCN.donghuaId AND TagCN.name = DonghuaTagCN.tagName WHERE TagCN.name IN ${sqlTagsList} OR TagCN.nameEN IN ${sqlTagsList})` : ""}`
  ...
  const results = await prisma.$queryRawUnsafe<Donghua[]>
    (`SELECT * FROM Donghua WHERE (${searchRelevanceSql}) > 0${filterSql} ${orderByFilter} LIMIT ${filter.limit || 10} OFFSET ${filter.offset || 0}`)
  const [{count: totalBigInt}] = await prisma.$queryRawUnsafe<[{count: BigInt}]>
    (`SELECT COUNT(*) AS count FROM Donghua WHERE (${searchRelevanceSql}) > 0${filterSql}`)
